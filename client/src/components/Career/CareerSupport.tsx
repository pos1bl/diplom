...
import { StyledSupport, StyledSupportImageBox, StyledInfoBlock } from '@components/styled/career';
import { SUPPORT_CARDS_LIST } from '@utils/Careerpage';
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3} justifyContent="center">
          {SUPPORT_CARDS_LIST.map(({ id, title, icon: Icon, text, link }) => (
            <Card key={id} sx={{  borderRadius: 3, boxShadow: 2 }}>
              <CardContent>
                <Typography variant="h6" fontWeight={700} mb={1} color='#A891D2'>
                  {title}
                </Typography>
                <a href={link} target='_blank'>
                  <Stack direction="row" alignItems="center" spacing={1}>
                    <Icon fontSize="small" sx={{ color: '#A891D2' }} />
